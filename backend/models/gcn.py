"""
Graph Convolutional Network (GCN) model for recommendation.
"""
import os
import torch
...
class GCNModel(torch.nn.Module):
    """
    GCN model for recommendation.
    """
    def __init__(self, input_dim, embedding_dim=128, num_layers=2):
        super().__init__()
        ...
        self.decoder = GCNDecoder(hidden_channels=embedding_dim)
    ...
    def train(self, graph, epochs=100, batch_size=1024, learning_rate=0.001, device=None):
        ...
        logger.info(f"Training GCN model on {device} for {epochs} epochs")
